// =========================================
// Scroll Restoration (リロード時にトップに戻す)
// =========================================
...
document.addEventListener('DOMContentLoaded', () => {
	...
	const translations = {
		ja: {
			...
			navPartners: "スポンサー様",
			navApply: "お問い合わせ",
			heroSubtitle: "EST. 2024 / TOKYO",
			secDoctrine: "理念",
			aboutCore: "CORE PHILOSOPHY",
			aboutLead: "勝敗は通過点。私たちが目指すのは、まだ誰も辿り着いていない次世代のeスポーツの形。<br><br>個の力への依存から脱却し、編隊としての絶対的な判断速度と連携で世界を圧倒する。",
			secMembers: "チームメンバー",
		...
		en: {
			...
			aboutCore: "CORE PHILOSOPHY",
			aboutLead: "Victory is merely a waypoint. Our goal is to shape the next generation of esports that no one has yet reached.<br><br>Breaking away from reliance on individual skill, we aim to overwhelm the world with absolute decision-making speed and coordination as a unified formation.",
	...
	langToggle.addEventListener('click', () => {
		...
		langToggle.querySelector('.icon').textContent = currentLang === 'en' ? 'EN' : 'JP';
	...
	if (typeof THREE !== 'undefined' && document.getElementById('webgl-container')) {
		...
		const camera = new THREE.PerspectiveCamera(120, initWidth / initHeight, 0.1, 1000);
		...
		for(let i = 0; i < particlesCount; i++) {
			...
			const ySpread = Math.pow(Math.random(), 2) * (Math.random() < 0.5 ? 1 : -1) * (10 / (r + 1.5));
		...
		particleMaterial = new THREE.PointsMaterial({
			size: 0.015, 
			color: htmlEl.getAttribute('data-theme') === 'dark' ? colorDark : colorLight,
			transparent: true, 
		...
		if (aboutSection && !reduceMotion) {
			...
			function updateAboutParallax() {
				...
				const progress = Math.min(Math.max(1 - (rect.top + rect.height * 0.5) / (vh + rect.height * 0.5), 0), 1);
		...
		if ('IntersectionObserver' in window && !reduceMotion) {
			const doctrineObserver = new IntersectionObserver((entries) => {
	...
	if(contactForm) {
		...
		contactForm.addEventListener('submit', async (e) => {
			...
			// 必須項目が空欄、または想定外のカテゴリーが送られてきた場合はエラーとして弾く
			if (!typeVal || !validTypes.includes(typeVal) || !nameVal || !emailVal || !messageVal) {
				alert("処理が実行できませんでした");
				return; // ここで処理を完全停止（通信をさせない）
			}
			// ----------------------------------------------------
			...
			try {
				...
				// Formspree等のAPIが200 OKを返した場合
				if (response.ok) {
					if (modalOverlay) modalOverlay.classList.add('is-visible');
					contactForm.reset();
				} else {
					// 400や500エラー時（サーバーからのエラー内容を直接表示しないことでXSS等のインジェクションを防止）
					alert("処理が実行できませんでした");
				}
			} catch (error) {
				console.error(error);
				// プレースホルダーのままテスト送信した場合のフォールバック動作
				if (contactForm.action.includes('your-form-id')) {
					if (modalOverlay) modalOverlay.classList.add('is-visible');
					contactForm.reset();
				} else {
					// ネットワークエラー等の場合も固定メッセージを使用
					alert("処理が実行できませんでした");
				}
			} finally {
				if(btn) {
	...
	if (sliderContainer && track) {
		...
		// Initial start after layout calculation
		setTimeout(() => {
			updateSlider();
			startAutoPlay();
		}, 100);
